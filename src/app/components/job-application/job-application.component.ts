...
formData.append('firstName', this.submitForm.get('firstName')?.value ?? ''); // Default to empty string if undefined
formData.append('lastName', this.submitForm.get('lastName')?.value ?? '');
formData.append('citizenship', this.submitForm.get('citizenship')?.value ?? '');
formData.append('dateOfBirth', this.submitForm.get('dateOfBirth')?.value ?? '');
...
formData.append('phoneNumber', this.submitForm.get('phoneNumber')?.value ?? '');
formData.append('email', this.submitForm.get('email')?.value ?? '');
formData.append('coverLetter', this.submitForm.get('coverLetter')?.value ?? '');
  ...
  showErrors() {
    const controlOrder = [
      'fullName', 'lastName', 'citizenship', 'dateOfBirth', 
      'address', 'zipCode', 'city', 'phoneNumber', 'email', 'resume'
    ]; // Order of controls
    ...
    for (const name of controlOrder) {
      ...
      if (control && control.invalid) {
        ...
        if (control.errors?.['required']) {
          ...
          this.snackBar.open(`${label} cannot be longer than ${maxLength} characters`, 'Close', {
            duration: 3000,
            verticalPosition: 'top',
            horizontalPosition: 'right',
          });
          ...
          this.snackBar.open(`${label} must be at least ${minLength} characters`, 'Close', {
            duration: 3000,
            verticalPosition: 'top',
            horizontalPosition: 'right',
          });
